...
**Date:** 2026-09-14 (updated: Slice 2 #455 + transcript split #461 merged)  
**Baseline:** `main` @ `55d75fb6` (`#461`) + `2af82b82` (`#455`)  
...
Complements `docs/upgrade-audit.md` (#389, LLM/security). Slice 1 **[#453](https://github.com/davins56/Anima-Protocol/pull/453)** and Slice 2 **[#455](https://github.com/davins56/Anima-Protocol/pull/455)** (`2af82b82`: 1024 clamp + `llmChatMessagesOpenTimeoutMs()` 18s) are on `main`. Local Ollama honors `req.maxTokens` (**[#457](https://github.com/davins56/Anima-Protocol/pull/457)**). Fat-prompt IMAGE/EMOTION survive **[#456](https://github.com/davins56/Anima-Protocol/pull/456)** / **[#461](https://github.com/davins56/Anima-Protocol/pull/461)** (split at contract markers, not the first blank line). Remaining Chat.jsx thinning is **[#458](https://github.com/davins56/Anima-Protocol/pull/458)**.

---

## 0. Latency verdict (read this first)

Chat **does stream**. The UI is not waiting for a full reply before painting tokens (`streamChatReply` + SSE deltas). After [#453](https://github.com/davins56/Anima-Protocol/pull/453), the **first SSE byte is a heartbeat after `beginChatTurn`**, not after context load. Companion turns cap at **1024** tokens (#455) and open in **18s** (`llmChatMessagesOpenTimeoutMs`) instead of the **80s** `:free` cascade. Local Ollama honors `req.maxTokens` (#457). Remaining slowness is **client-side fat `systemPrompt` / CHARACTER prefill** until [#458](https://github.com/davins56/Anima-Protocol/pull/458).
...
**Shipped (fat-prompt contracts):** [#456](https://github.com/davins56/Anima-Protocol/pull/456) `f9db61c7` keeps IMAGE/EMOTION/LOCATION after `Story so far:`. [#461](https://github.com/davins56/Anima-Protocol/pull/461) `55d75fb6` splits at contract markers (`INTELLIGENCE:`, `[IMAGE:`, …), not the first blank line inside a user message.
...
### P0-L3 — Cap generation length; don’t use the 80s free-tier open budget on Chat.jsx — **shipped in #455 + #457**
...
`retrieveRelevantMemories` is in-process scoring (topK 12, last 24 turn crumbs). `attachStoredEmbeddings` is a DB read of JSON vectors. That is cheaper than leftover Chat.jsx fat prompt. **Do not** “fix speed” by deleting companion memory. P0-L1/L2/L3 server side shipped. Identity-loop P1 still matters for *quality* of recall, not the first-token budget.
...
| [#458](https://github.com/davins56/Anima-Protocol/pull/458) Lean 1:1 Chat.jsx + TTFT caps | **Open.** Rebase onto `main` (now has #455+#456+#457+#461). Conflicts were on `chat.ts` / `llmFailover.ts` / `chatTimeouts.ts`. Later commits on that branch already keep `CORE_BEHAVIOR`, reserve trailing lean extras, await first-turn `AIBehaviorConfig`, include user profile, and keep OpenRouter out of ensemble minds. Still drops client `CharacterMemory` without a distillation merge. Group path still fat. |
...
Do not duplicate. Remaining Chat.jsx thinning is in [#458](https://github.com/davins56/Anima-Protocol/pull/458) — rebase that onto `main`; do not start a third token-cap PR.

### Slice 2 — E2E: honor token cap on local Ollama; stop 80s OpenRouter cascade (P0-L3) — **shipped in #455 + #457**
...
| Client wrap ≤2k after stripping transcript | `promptBuilder.ts` `clientSceneExcerpt` / `CLIENT_SCENE_CONTEXT_MAX`; #456 keeps IMAGE/EMOTION; #461 splits at contract markers |
...
| `/chat/messages` 18s open + 1024 cap | #455 `llmChatMessagesOpenTimeoutMs` + `clampChatMessagesMaxTokens`; `chat.ts` |
