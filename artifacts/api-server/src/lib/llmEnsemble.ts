...
function resolveMindModel(provider: LlmProviderId, tier: ModelTier) {
  if (provider === "openrouter") return resolveOpenRouterModel(tier);
...
function pickSynthesizer(drafts: MindDraft[]): LlmProviderId {
  const order: LlmProviderId[] = ["local", "openrouter"];
  ...
  return [
    {
      ...
      content:
        ...
        `from those drafts. Do not mention the minds, drafts, or that you are combining answers. Stay fully in character. Output only the final reply.`,
    },
    ...userTurns,
    {
      role: "user",
...
async function synthesizeDrafts(
  ...
  const client = synthesizer === "openrouter" ? getOpenRouterClient() : getOpenAIClient();
  if (!client) {
...
export async function createEnsembleChatReply(
  ...
  if (minds.length === 0) {
    throw new Error(
      "No LLM minds configured for ensemble.",
    );
  }
