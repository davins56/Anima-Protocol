...
- `/api/__clerk/npm/@clerk/clerk-js@6/dist/clerk.browser.js` must return **200 JavaScript**. Clerk CDN 307s the `@6` dist-tag to a patched version; the Worker follows that hop. `coerceApiResponseToJson` must not rewrite 3xx (that dropped `Location` and broke the script tag). GitHub OAuth App callback is `https://clerk.anima-protocol.com/v1/oauth_callback` — see `scripts/cloudflare/clerk-oauth.md`. That hostname is **gatewayed** by Worker `anima-protocol` (`wrangler.jsonc` routes + `clerkCnameGateway`) so iPad Safari does not render Clerk's `authorization_invalid` JSON. Do not orange-cloud the raw Clerk CNAME (Error 1014). Do not drop the route.
