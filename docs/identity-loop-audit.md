...
**Date:** 2026-09-14 (updated: #454/#456/#457 merged)  
**Baseline:** `main` @ `b59c2db5` (`#457` local `max_tokens`) + `f9db61c7` (`#456` excerpt suffix) + `95283f32` (`#454` this audit)  
...
Complements `docs/upgrade-audit.md` (#389, LLM/security). **[#453](https://github.com/davins56/Anima-Protocol/pull/453) merged** Slice 1. **[#456](https://github.com/davins56/Anima-Protocol/pull/456) merged** keeps IMAGE/EMOTION/LOCATION after `Story so far:`. **[#457](https://github.com/davins56/Anima-Protocol/pull/457) merged** honors `req.maxTokens` on local Ollama. Remaining Slice 2 on open [#455](https://github.com/davins56/Anima-Protocol/pull/455): 1024 clamp + 18s `/chat/messages` open (rebase; do not re-land the local cap). Lean 1:1 Chat.jsx is still [#458](https://github.com/davins56/Anima-Protocol/pull/458).

---

## 0. Latency verdict (read this first)

Chat **does stream**. The UI is not waiting for a full reply before painting tokens (`streamChatReply` + SSE deltas). After [#453](https://github.com/davins56/Anima-Protocol/pull/453), the **first SSE byte is a heartbeat after `beginChatTurn`**, not after context load. Remaining slowness is **client-side fat `systemPrompt` build**, **prefill still from CHARACTER / memories / CORE**, then **generation still routed at 4–8k until #455’s 1024 clamp lands**, then an **80s OpenRouter `:free` cascade** if local hops. Local Ollama now honors `req.maxTokens` (#457).
...
**Shipped (fat-prompt contracts):** [#456](https://github.com/davins56/Anima-Protocol/pull/456) `f9db61c7` splits `clientSceneExcerpt` at the first blank line after `Story so far:` / `CONVERSATION CONTEXT:` and prefers the suffix when capping 2k, so IMAGE/EMOTION/LOCATION survive.

**Still open:** Chat.jsx still concatenates character sheet + 14×800 history + lore + CharacterMemory before POST. Thinning that payload is identity/P1 work in [#458](https://github.com/davins56/Anima-Protocol/pull/458) (lean 1:1 extras). Group still uses fat `buildGroupPrompt`.
...
**Where:** `modelRouter.ts` `MAX_TOKENS`; `llmFailover.ts` `cappedLocalMaxTokens` + `localAttemptSignal`; `chat.ts` `openStreamAbort` / `createChatStreamWithFailover({ signal: open.signal })`; `chatTimeouts.ts`; `animaApi.js` `CHAT_STREAM_TIMEOUT_MS=130_000`; `wrangler.jsonc` fallback/free vars.
...
| Item | Status |
|------|--------|
| [#458](https://github.com/davins56/Anima-Protocol/pull/458) Lean 1:1 Chat.jsx + TTFT caps | **Open.** Rebase onto `main` (now has #456+#457) **and** onto #455 if that still lands first. Conflicts with #455 on `chat.ts` / `llmFailover.ts` / `chatTimeouts.ts`. Later commits on that branch already keep `CORE_BEHAVIOR`, reserve trailing lean extras, await first-turn `AIBehaviorConfig`, include user profile, and keep OpenRouter out of ensemble minds. Still drops client `CharacterMemory` without a distillation merge. Group path still fat. |
| [#457](https://github.com/davins56/Anima-Protocol/pull/457) Honor `req.maxTokens` on local Ollama | **Merged** `b59c2db5` (2026-09-14) even though it was marked superseded by #455. Local `cappedLocalMaxTokens` is on `main`. Do not merge #455’s overlapping `llmFailover.ts` hunks without rebase. |
| [#456](https://github.com/davins56/Anima-Protocol/pull/456) Keep IMAGE/EMOTION after `Story so far:` | **Merged** `f9db61c7` (2026-09-14). Needed for current fat Chat.jsx and group `buildGroupPrompt` until #458 lands. |
| [#455](https://github.com/davins56/Anima-Protocol/pull/455) Slice 2 E2E — 1024 clamp + 18s open | **Open** (ready). Remaining unique work is the 1024 companion clamp + 18s `/chat/messages` open. Local `max_tokens` already shipped in #457 — rebase before merge. |
...
| [#450](https://github.com/davins56/Anima-Protocol/pull/450) Worker ETIMEOUT ≠ DB; `/api/ai/chat` 18s; **12s `localAttemptSignal` in shared failover**; probe 45s | **Merged** `f4a7010a` (2026-09-14). Do not re-open classification. Do **not** recommend another 12s hop on `/chat/messages`. Remaining Chat.jsx work: 1024 clamp + 80s OpenRouter outer abort (#455). |
| Rate-limit | Merged #125. User-keyed. Leave it. |
| `main` CI `api-tests` | `llmEnsemble.test.ts` vs OpenRouter in CI chain. [#458](https://github.com/davins56/Anima-Protocol/pull/458) `17da2ef6` keeps OpenRouter out of parallel ensemble minds — not on `main` yet. |
...
Do not duplicate. Remaining Chat.jsx thinning is in [#458](https://github.com/davins56/Anima-Protocol/pull/458) — rebase that onto `main` (and #455 if still open); do not start a third token-cap PR.
...
**After #455:** identity Slice A — seed `companion_memories` on create, then **migrate** distilled `CharacterMemory` facts into that store (or the server prompt) **before** dropping the Chat.jsx block. [#458](https://github.com/davins56/Anima-Protocol/pull/458) already drops the client memory block on solo. Do **not** add `GET /chat/memories` to the client prompt.
...
- Ensemble / OpenRouter chain CI (`llmEnsemble.test.ts` still red on `main`; fix lives on #458 `17da2ef6`).
...
| Client wrap ≤2k after stripping transcript | `promptBuilder.ts` `clientSceneExcerpt` / `CLIENT_SCENE_CONTEXT_MAX`; #456 keeps post-transcript IMAGE/EMOTION/LOCATION |
...
| #450 merged | `origin/main` `f4a7010a`; `chat.ts` still `llmOpenTimeoutMs({ freeTierCascade: usesFreeTierOpenBudget() })` until #455 |
