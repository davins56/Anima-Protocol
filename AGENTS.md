...
- **Workers AI 4006 (daily free neurons):** `POST /api/ai/chat` returning 502 `ai_request_failed` while `/api/healthz/llm` still shows `preferred: workersai` / `configured: true` is usually Cloudflare error **4006** — free-plan 10k neurons/day exhausted, not a missing AI binding or a Cloud Run prep (#442) regression. `GET /api/healthz/llm?probe=1` live-runs the model and should report `errorKind: quota`. Signed-in chat (`POST /api/chat/messages` via `createChatStreamWithFailover`) uses the same `aiBinding.run` and fails the same way. **Do not silently change the preferred provider.** Options (Dàvīn approval required to switch chat off Workers AI): (1) enable **Workers Paid** so DeepSeek stays preferred; (2) temporarily allow OpenRouter (`ANIMA_OPENROUTER_FREE` / lift the `ANIMA_LLM_PROVIDER=custom` `customOnly` pin) — must be labeled and approved.
