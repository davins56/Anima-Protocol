...
| `KIMI_API_KEY` / `MOONSHOT_API_KEY` | API | Optional. Kimi (Moonshot). Default when Gemini is unset. Force with `ANIMA_LLM_PROVIDER=kimi` |
| `XAI_API_KEY` | API | Optional. Grok (xAI) — used under `ANIMA_LLM_PROVIDER=auto` / `xai`, not as a Gemini/Kimi-mode backup |
...
| `ANIMA_LLM_PROVIDER` | API | Unset: Kimi-only if `KIMI_API_KEY`, else Gemini-only if `GEMINI_API_KEY`, else `auto`. Or `kimi` / `moonshot` / `gemini` / `auto` / `xai` / `openai` |
