...
- Workers Builds **main** runs `npx wrangler deploy` (GET `/workers/scripts/anima-protocol/deployments` first). PR previews run `wrangler versions upload` and skip that GET. Wrangler 4.129+ jsonc-parses the 200 body; a malformed payload aborts production **after** `pnpm build` (PR #401 merge `d9d881b`, build `d2ee859e-8544-4d56-9598-f70cba9e7448`). Keep `scripts/cloudflare/install-wrangler-deploy-guard.mjs` in `postinstall` — see `scripts/cloudflare/workers-builds-deploy.md`.
