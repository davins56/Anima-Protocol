...
**Date:** 2026-09-14 (updated: #480 Worker subrequest HUD)  
**Baseline:** `main` @ `19f280c8` (`#480`) / `f7d3d833` (`#478`)  
...
| Cold Ollama | **True for ops; 12s hop only if a next provider exists.** After #476: Ollama `keep_alive: "10m"` (opt-out `ANIMA_OLLAMA_KEEP_ALIVE=off`). After #480 the native `/api/generate` warm hint is **skipped on Cloudflare Workers** (it burned the Worker subrequest budget). `ANIMA_LOCAL_LLM_MAX_RETRIES` defaults to **0** on Workers / Vercel / Cloud Run and **2** on local Node. `localAttemptSignal` combines the caller abort with `LLM_LOCAL_FAILOVER_ATTEMPT_MS` (**12s**) whenever a next provider exists. **#464:** a usable custom host makes `getProviderChain()` `[local]` only — local connection/timeout returns `ai_timeout` / connection error, no OpenRouter hop. Do not set `ANIMA_OPENROUTER_FALLBACK` to paper over a down self-hosted host. |
...
**Ops (not a code PR):** host-side warmup cron against `llm.anima-protocol.com` still helps cold boxes; in-app `keep_alive: "10m"` is already #476. Worker native `/api/generate` warm is skipped (#480). Worker retries already default to **0** (#480); `ANIMA_LOCAL_LLM_MAX_RETRIES` still overrides. Worker can set `ANIMA_OPENROUTER_FALLBACK=false` to drop the remaining Workers AI → OpenRouter hop.
...
| Progress SSE + keep_alive | #476 JSON `{ status: "progress", phase }`; Ollama `keep_alive: "10m"`; `llmChatMessagesStreamTotalMs()` **75s** (130 − 45 − 10). Client abort is `CHAT_STREAM_TIMEOUT_MS` 130s in `animaApi.js`. #480 skips Worker native `/api/generate` warm. |
