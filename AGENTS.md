...
**Anima Protocol** is a pnpm monorepo: React/Vite frontend (`artifacts/anima-protocol`), Express API (`artifacts/api-server`), shared Drizzle DB package (`lib/db`), and optional **Mockup Sandbox** (`artifacts/mockup-sandbox`) for isolated UI previews at `/__mockup`. Kernel / Operator Model design: `artifacts/anima-protocol/docs/ANIMA_KERNEL.md`.
...
| `VITE_CLERK_PROXY_URL` | Explicit proxy URL, or `none` / `false` / `off` to disable all proxying. When unset, `pk_live_` on `anima-protocol.com` **always** uses same-origin `/api/__clerk/` — including custom-domain keys (`clerk.anima-protocol.com`). Direct CNAME FAPI skips the Worker cookie strip and is the post-GitHub `authorization_invalid` path. Do not omit `proxyUrl` on ClerkProvider in production. |
...
| `MINIMAX_API_KEY` / `ANIMA_MINIMAX_API_KEY` | API chat via MiniMax Global. Preferred over OpenRouter when configured; defaults to `MiniMax-M2.7` (optional `ANIMA_MINIMAX_MODEL` and `ANIMA_MINIMAX_BASE_URL` overrides) |
...
**Production (Cloudflare — anima-protocol.com):** the apex host is Workers + Assets, not Vercel. Root `wrangler.jsonc` sets `main` to `artifacts/api-server/src/worker.ts` with `nodejs_compat`. Workers Builds already runs `pnpm build` (copies the SPA to `./dist`) then `npx wrangler deploy --assets=./dist --name anima-protocol`. `assets.run_worker_first` sends `/api` and `/api/*` to Express so `/api/healthz`, `/api/store`, and `/api/__clerk` are not swallowed by the SPA fallback, and `/assets` / `/assets/*` to the Worker so a missing hashed file returns 404 instead of `index.html`. Keep Clerk/DB/OpenRouter keys **and** the local Anima LLM URL/key in Cloudflare **Secrets Store** (store `a31e40473ef34db896b5bc1e6c1c4b86`) and declare them in `wrangler.jsonc` `secrets_store_secrets` (`CLERK_SECRET_KEY`, `CLERK_PUBLISHABLE_KEY`, `DATABASE_URL`, `OPENROUTER_API_KEY`, `ANIMA_LOCAL_LLM_BASE_URL`, `ANIMA_LOCAL_LLM_API_KEY`). `ANIMA_LOCAL_LLM_BACKEND=ollama` stays in `vars` only — the same name in Secrets Store fails `wrangler deploy` ("Bindings must have unique names"). `ANIMA_LLM_PROVIDER=custom` is a committed `vars` pin (chat is fail-closed). **Create those LLM `secret_name`s in the store before deploying** — a binding for a missing entry fails `wrangler deploy` and blocks the Worker. See `deploy/ollama-fly/README.md`. Commit binding names and `store_id` only — never secret values. A git deploy that omits `secrets_store_secrets` unbinds dashboard secrets and `/api/healthz/env` goes all-false. The Worker never invents `http://localhost:11434/v1`; `/api/healthz/llm` reports `localEndpoint.configured: false` until a public HTTPS `…/v1` URL is bound. See `deploy/ollama-fly/README.md`. A deploy without `main` is assets-only and those `/api` paths 404. **Postgres from the Worker:** keep the Secrets Store `DATABASE_URL` binding; a raw `pg` TCP Pool against that secret from Workers still ECONNRESET. The Worker uses postgres.js and a `HYPERDRIVE` binding. One dashboard step: create Hyperdrive, paste `DATABASE_URL` **into that form once** (never into git), then put only `{ binding, id }` in `wrangler.jsonc` and redeploy — see `scripts/cloudflare/hyperdrive.md`.

**Production (recommended on Vercel):** set `VITE_CLERK_PUBLISHABLE_KEY` and `CLERK_PUBLISHABLE_KEY` to matching **`pk_live_` / `sk_live_`** from the **same** Clerk Production app (never put `sk_` in `VITE_CLERK_PUBLISHABLE_KEY`). This deployment uses Clerk **custom domain** (`clerk.anima-protocol.com` CNAME) — leave `VITE_CLERK_PROXY_URL` empty so the SPA auto-sets `proxyUrl=/api/__clerk/` (required for Safari ITP first-party cookies). clerk-js without `proxyUrl` calls `https://clerk.anima-protocol.com/v1/*` directly and GitHub → sso-callback handshake cannot use the Worker strip. Verify `curl https://www.anima-protocol.com/api/healthz` and `curl https://clerk.anima-protocol.com/v1/environment` return 200 after deploy. Register OAuth redirect URLs (`…/sign-in/sso-callback`, `…/sign-up/sso-callback`) in Clerk → Paths. Redeploy **without build cache** after any env change. `/api/__clerk` is also used on `*.vercel.app` when you set an explicit proxy URL.
...
- Companion store "Postgres is unreachable from this host" on anima-protocol.com means the Worker has `DATABASE_URL` but no Hyperdrive path to Postgres. Create/bind Hyperdrive (`scripts/cloudflare/hyperdrive.md`); do not put the URL in `wrangler.jsonc` vars. Hyperdrive origin must be real Postgres, not Prisma Accelerate (`db.prisma.io`). A "database unavailable" toast is this `/api/store` 503 / Hyperdrive path — do not conflate it with Workers AI quota.
- **Custom Anima LLM:** Chat prefers the self-hosted `anima-chat` model at `ANIMA_LOCAL_LLM_BASE_URL` (production: `https://llm.anima-protocol.com/v1`). Do not prefer Workers AI DeepSeek or OpenAI when that URL is usable. HTTP 530 / Cloudflare 1033 on that host means the tunnel origin is down — start Ollama + cloudflared, do not silently switch back to DeepSeek. `customOnly` is fail-closed: OpenRouter is **not** appended after a usable custom host, even when `ANIMA_OPENROUTER_FALLBACK` and the key are bound. That flag only hops after Workers AI 4006 when no custom host is set. Leave it `false` for custom-only operation; do not set it to paper over a down local host.
- **Workers AI 4006 (daily free neurons):** Only applies when the custom host is unset/unusable and the chain still includes `workersai`. `POST /api/ai/chat` returning 502 `ai_request_failed` while `/api/healthz/llm` shows `preferred: workersai` / `configured: true` is usually Cloudflare error **4006** — free-plan 10k neurons/day exhausted. `GET /api/healthz/llm?probe=1` live-runs the model and should report `errorKind: quota`.
- www 301 to `https://anima-protocol.com/` (path dropped) is zone Redirect Rule **"Redirect www to root"** (`scripts/cloudflare/www-redirect.md`), not `vercel.json`. Fix the replacement to `https://anima-protocol.com/${1}`. Apex `/api/*` must still return JSON when the isolate throws. Apex **MUST** stay in `wrangler.jsonc` `routes` as `{ "pattern": "anima-protocol.com", "custom_domain": true }` alongside clerk. A deploy that lists only clerk deletes the apex Custom Domain and its proxied A/AAAA (site NXDOMAIN; www still 301s nowhere). Do not add www as a custom domain.
- `/api/__clerk/npm/@clerk/clerk-js@6/dist/clerk.browser.js` must return **200 JavaScript**. Clerk CDN 307s the `@6` dist-tag to a patched version; the Worker follows that hop. `coerceApiResponseToJson` must not rewrite 3xx (that dropped `Location` and broke the script tag). GitHub OAuth App callback is `https://clerk.anima-protocol.com/v1/oauth_callback` — see `scripts/cloudflare/clerk-oauth.md`. That hostname must be a Worker **Custom Domain** (`custom_domain: true` + `clerkCnameGateway`). A zone route does not receive grey-cloud CNAME traffic. Delete `CNAME clerk → frontend-api.clerk.services` before deploy (`scripts/cloudflare/clerk-cname-gateway.md`). Do not orange-cloud the Clerk CNAME (Error 1014).
- Workers Builds **main** runs `npx wrangler deploy` (GET `/workers/scripts/anima-protocol/deployments` first). That GET on #401 (`d9d881b`, build `d2ee859e`) aborted **before** asset upload. #402 (`bd3ea40`, build `83c54401`) recovered it and deployed version `7a7d1ec0` — production `clerk-js@6` is 200 JS. PR previews run `wrangler versions upload` and can still fail **after** a successful upload on GET `/workers/subdomain` 503. Keep `scripts/cloudflare/install-wrangler-deploy-guard.mjs` in `postinstall` — see `scripts/cloudflare/workers-builds-deploy.md`.
